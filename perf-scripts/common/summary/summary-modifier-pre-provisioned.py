...
# Write Scenario name, Concurrent Users, Error %, Throughput (Requests/sec), Average Response Time (ms) into a new file
with open('updated_summary.csv', 'w') as file:
    writer = csv.writer(file)
    writer.writerow([rows[0][0], rows[0][2], rows[0][6], rows[0][7], rows[0][8]])   # Write column names
    ...
    for count in scenarioCount:
        for i in range(concurrentUserCounts):
            ...
            # 0 - Scenario name, 2 - Concurrent users, 6 - Error %, 7 - Throughput (Requests/sec), 8 - Average Response Time (ms)
            # Error %, Throughput and response time are rounded to first two decimal places
            # Read column wise for getting average error %, throughput and total of average response times using numpy nd arrays
            writer.writerow(
                [rows[rowNumber][0], rows[rowNumber][2],
                 round(float(np.mean([float(i) for i in np.array(rows)[:, 6][rowNumber:rowNumber + stepsCount]])), 2),
